...
def ServoJ(j0, j1, j2, j3, j4, j5, t1, t2, gain, alpha):
    msg = 'move_servo_j(jnt[' + str(j0) + ',' + str(j1) + ',' + str(j2) + ',' + str(j3) + ',' + str(
        j4) + ',' + str(j5) + '], ' + str(t1) + ',' + str(t2) + ',' + str(gain) + ',' + str(alpha) +')'
    print(time.time()*1000, flush=True)
    print(msg, flush=True)  # 버퍼 강제 플러시
    return SendCOMMAND(msg, CMD_TYPE.MOVE)

@dispatch(float, float, float, float, float, float, float, float, float, float)
def SpeedJ(dj0, dj1, dj2, dj3, dj4, dj5, t1, t2, gain, alpha):
    msg = 'move_speed_j(jnt[' + str(dj0) + ',' + str(dj1) + ',' + str(dj2) + ',' + str(dj3) + ',' + str(
        dj4) + ',' + str(dj5) + '], ' + str(t1) + ',' + str(t2) + ',' + str(gain) + ',' + str(alpha) +')'
    #print(msg)
    return SendCOMMAND(msg, CMD_TYPE.MOVE)
...
def ReqDataStart(sock):
    while True:
        try:
            ...
            print(f"[ERROR] Connection lost in ReqDataStart: {e}")
            break
...
def ReadDATA(sock):
    ...
    while True:
        try:
            ...
            if not msg_recv or len(msg_recv) == 0:
                print("[ERROR] No data received, connection may be closed")
                break
        ...
        except (ConnectionResetError, IndexError, OSError) as e:
            print(f"[ERROR] Connection lost in ReadDATA: {e}")
            break
        except Exception as e:
            print(f"[ERROR] Unexpected error in ReadDATA: {e}")
            break

        if cmd_connect == 1 & data_connect == 1:
            break

_send_min_interval_s = 0.0
_last_send_t = 0.0
_send_lock = threading.Lock()

def set_send_min_interval(dt_s: float):
    global _send_min_interval_s
    try:
        _send_min_interval_s = max(0.0, float(dt_s))
    except Exception:
        _send_min_interval_s = 0.0
...
def SendCOMMAND(str, cmd_type):
    ...
    if cmd_type == CMD_TYPE.MOVE:
        ...
        try:
            with _send_lock:
                now = time.monotonic()
                if _send_min_interval_s > 0.0:
                    wait = (_last_send_t + _send_min_interval_s) - now
                    if wait > 0:
                        time.sleep(wait)
                        now = time.monotonic()
                # 업데이트
                globals()['_last_send_t'] = now
